...
def predict(img_path, knn_clf=None, model_path=None, threshold=0.6):  # 6 needs 40+ accuracy, 4 needs 60+ accuracy
    ...
    accu = [120 - closest_distances[0][i][0] * 100 for i in range(len(face_box))]
    # Predict classes and remove classifications that aren't within the threshold
    return [(pred, loc, acc) if rec else ("unknown", loc, acc) for pred, loc, rec, acc in
            zip(knn_clf.predict(faces_encodings), face_box, matches, accu)]
...
while True:
    ...
    predictions = predict(frame_copy, model_path="Classifier/trained_knn_model.clf")  # add path here
